  ...
  <div class="col-md-6 p-0">
    <div class="half">
      <div class="login-logo">
        <a class="" href="{{ url('/') }}">
          <img src="{{ asset(config('app.logo_frontend'))}}" height="50" alt="logo">
        </a>
      </div>
      <form class="actionLogin mb-2" action="{{ route('register') }}" method="post">
        ...
        <div class="form-group first">
          <input class="form-control" type="text" name="mobile"  value="{{ old('mobile') }}" placeholder="Enter Mobile Number" required="">
        </div>
        <div class="form-group first">
       ...
       <div class="form-group">
        <select name="role" class="form-control">
            ...
            <option value="candidate">@lang('Register as Candidate')</option>
            <option value="employer">@lang('Register as Employer')</option>
        </select>
        </div>
       <!--<input type="hidden" value='candidate' name="role"/>-->
        @if(config('recaptcha.api_site_key') && config('recaptcha.api_secret_key'))
        <div class="form-group">
          {!! htmlFormSnippet() !!}
      ...
      <p class="mt-2">
        <a href="{{ route('login') }}">@lang('Already have account login?')</a>
      </p>
    </div>
  </div>
