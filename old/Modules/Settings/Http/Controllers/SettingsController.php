...
class SettingsController extends Controller
{
    ...
    public function dashboard(Request $request)
    {
        ...
        $total_candidates = User::candidate()->count();

        $data_jobs = Job::orderBy('created_at', 'DESC');
        $total_jobs = $data_jobs->count();
        $recent_20_jobs = $data_jobs->take(20)->get();

        $data_companies = Company::orderBy('created_at', 'DESC');
        $total_companies = $data_companies->count();
