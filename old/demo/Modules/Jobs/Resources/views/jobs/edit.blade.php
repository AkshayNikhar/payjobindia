...
<div class="d-sm-flex align-items-center justify-content-between mb-4">
    ...
    <div class="ml-auto">
          <a href="{{route('job',$job->slug)}}" class="btn btn-primary">
            <i class="far fa-eye"></i> @lang('Job public')</a>
    </div>
</div>
<div class="row">
    <div class="col-md-12">

        <form role="form" method="post" action="{{ route('settings.jobs.update', $job) }}">
            @method('PUT')
            @csrf
            <div class="card">
                <div class="card-body">
                    ...
                    <div class="row">
                        <div class="col-md-12">
                            <div class="form-group">
                                <label class="form-label required">@lang('Company')</label>
                                <select name="company_id" class="form-control border-dark" required="">
                                    @foreach($companies as $company)
                                        <option value="{{ $company->id }}" {{ $job->company_id == $company->id ? 'selected' : '' }}>{{ $company->company_name }}</option>
                                    @endforeach
                                </select>
                            </div>
                        </div>
...
@push('scripts')
<script src="{{ Module::asset('jobs:js/jobs.js') }}"></script>
<script>
